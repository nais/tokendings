...
| `AUTH_PROVIDER_CONFIGS` | No | JSON array of external provider configs (see below). Mutually exclusive with `AUTH_WELL_KNOWN_URL` |
...
In the Helm chart deployment, `AUTH_PROVIDER_CONFIGS` is auto-generated from `authWellKnownUrl` with a default `allowedSubjects` of `["system:serviceaccount:nais-system:tokenx-jwker"]` and no cluster name restriction. Set `authProviderConfigs` explicitly to override this.
