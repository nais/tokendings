...
|----------------------------------------|------------------------------------------------------------------------|------------------------------------------------------------|
| **External (OIDC Metadata Discovery)** | `AUTH_PROVIDER_CONFIGS` or `AUTH_WELL_KNOWN_URL`                       | Per-provider cluster name restriction and allowed subjects |
...
| `AUTH_PROVIDER_CONFIGS`  | No       | JSON array of external provider configs (see below).                                                                 |
| `AUTH_WELL_KNOWN_URL`    | No       | *Legacy* — single external provider without restrictions. Ignored if `AUTH_PROVIDER_CONFIGS` is set.                 |
| `AUTH_CLIENT_ID`         | No       | Issuer name for self-signed mode (e.g. `jwker`). Ignored if `AUTH_PROVIDER_CONFIGS` or `AUTH_WELL_KNOWN_URL` is set. |
